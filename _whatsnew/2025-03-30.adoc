...
Mithilfe der Workload Factory API können Sie jetzt eine Datenquelle aus einer generischen NFSv3-, NFSv4- oder SMB-Freigabe hinzufügen.  Wenn Sie eine Datenquelle aus einer NFS- oder SMB-Freigabe hinzufügen, verbleibt das Wissensdatenbank-Volume auf einem Amazon FSx for NetApp ONTAP -Volume.  Die Workload Factory-Web-Benutzeroberfläche wird diese Funktion in einer zukünftigen Version unterstützen.

link:https://console.workloads.netapp.com/api-doc["Verwenden der Workload Factory-API"^]
